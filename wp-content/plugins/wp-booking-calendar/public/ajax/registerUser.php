		...
		if( null == email_exists( $email_address ) ) {
		  ...
		  ?>
			<script>
				window.parent.$wbc('#booking_sfondo').remove();
			  window.parent.$wbc('#booking_modal_loading').fadeOut();
			   window.parent.alert("<?php echo __( 'You\'ve been successfully registered', 'wp-booking-calendar' ); ?>");
			...
			<script>
			   window.parent.$wbc('#booking_sfondo').remove();
			   window.parent.$wbc('#booking_modal_loading').fadeOut();
			   window.parent.alert("<?php echo __( 'Existing user, please login', 'wp-booking-calendar' ); ?>");
