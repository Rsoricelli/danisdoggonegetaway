...
class wp_booking_calendar_public_reservation {
	...
	public function setReservationByMD5($id) {
		...
		$reservationQry = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".$wpdb->base_prefix.$blog_prefix."booking_reservation WHERE MD5(reservation_id) = %s",$id));
	...
	public function confirmReservations($listIds) {
		...
		$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_reservation SET reservation_confirmed = %d, reservation_fake = %d WHERE MD5(reservation_id) IN (".$listReservations.")",1,0));
	}
	...
	public function unfakeReservations($listIds) {
		...
		$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->base_prefix.$blog_prefix."booking_reservation SET reservation_fake = %d WHERE MD5(reservation_id) IN (".$listReservations.")",0));
	}
	...
	public function checkReservationPaypalPaid($listIds) {
		...
		$arrayReservations = explode(",",$listIds);
		for($i=0;$i<count($arrayReservations);$i++) {
			$checkQry=$wpdb->query($wpdb->prepare("SELECT * FROM ".$wpdb->base_prefix.$blog_prefix."booking_reservation WHERE MD5(reservation_id) = %s AND reservation_confirmed = %d AND reservation_fake = 0",$arrayReservations[$i],1));
			if($checkQry>0) {
				$count++;
			}
		}
