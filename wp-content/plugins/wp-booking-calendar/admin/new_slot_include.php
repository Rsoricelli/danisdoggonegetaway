...
<script>
	...
	function addTime(num) {
		...
		htmlToAdd = '<div id="time_'+num+'" class="booking_font_12"><br><br><?php echo __( 'From', 'wp-booking-calendar' ); ?>&nbsp;';
		...
		htmlToAdd+='<option value="">hour</option>';
                ...
                htmlToAdd+='&nbsp;';
               htmlToAdd+='&nbsp;';
				htmlToAdd+='<?php echo __( 'To', 'wp-booking-calendar' ); ?>&nbsp;';
	...
	function addTimeRange(num) {
		...
		htmlToAdd='<div id="time_range_'+num+'" class="booking_font_12"><br><br><div class="booking_float_left booking_margin_t_10"><?php echo __( 'From', 'wp-booking-calendar' ); ?></div>';
        ...
        htmlToAdd+='<option value="">hour</option>';
                ...
                htmlToAdd+='<div class="booking_float_left booking_margin_t_10 booking_margin_l_10"><?php echo __( 'To', 'wp-booking-calendar' ); ?></div>';
                htmlToAdd+='<div class="booking_float_left booking_margin_l_10">';
	...
	function checkData(frm) {
		with(frm) {
			if(slot_date_from.value=='') {
				...
				alert("<?php echo __( 'Choose slot interval', 'wp-booking-calendar' ); ?>");
				return false;
				...
				alert("<?php echo __( 'Insert a valid value for slot interval', 'wp-booking-calendar' ); ?>");
				return false;
				...
				alert("<?php echo __( 'Insert at least a custom slot', 'wp-booking-calendar' ); ?>");
				return false;
			} else if(slot_interval.options[slot_interval.selectedIndex].value == '0' && checkSlotIntervalCustomTimes()==1) {				
				alert("<?php echo __( 'Slot duration values are not correct', 'wp-booking-calendar' ); ?>");
				return false;
			} else if(slot_pause.value != '0' && slot_pause.value!= '' && (!isNumeric(slot_pause) || slot_pause.value<5 || slot_pause.value>1435)) {
				alert("<?php echo __( 'Insert a valid value for slot pause', 'wp-booking-calendar' ); ?>");
				return false;
				...
				alert("<?php echo __( 'Select at least time from and a time to', 'wp-booking-calendar' ); ?>");
				return false;
			} else if(slot_interval.options[slot_interval.selectedIndex].value == '1' && checkSlotTimes() == 1) {
				alert("<?php echo __( 'Time periods for slots are not correct', 'wp-booking-calendar' ); ?>");
				return false;
			} else {
