...
<script>
	...
	function showFilters(value) {
		if(value==1) {
			...
			$wbc('#label_from').html("<?php echo __( 'Day', 'wp-booking-calendar' ); ?>");
			$wbc('#date_to_filter').slideUp();
			$wbc('#weekdays_filter').slideUp();
		} else if(value==2) {
			$wbc('#filters').slideDown();
			$wbc('#label_from').html("<?php echo __( 'From', 'wp-booking-calendar' ); ?>");
	...
	function delItem(itemId,reservation) {
		if(confirm("<?php echo __( 'Are you sure you want to delete this slot?', 'wp-booking-calendar' ); ?>")) {
	...
	function saveItem(slot) {
		if(Trim($wbc('#slot_date_'+slot).val()) != '' && Trim($wbc('#slot_time_from_hour_'+slot).val()) != '' && Trim($wbc('#slot_time_from_minute_'+slot).val()) != '' && Trim($wbc('#slot_time_to_hour_'+slot).val()) != '' && Trim($wbc('#slot_time_to_minute_'+slot).val()) != '') {
			...
			$wbc.ajax({
			  url: '<?php echo plugins_url('wp-booking-calendar/admin/');?>ajax/saveSlot.php?item_id='+slot+"&date="+$wbc('#date_value_'+slot).val()+"&time_from="+$wbc('#slot_time_from_hour_'+slot).val()+":"+$wbc('#slot_time_from_minute_'+slot).val()+":"+$wbc('#slot_time_from_ampm_'+slot).val()+"&time_to="+$wbc('#slot_time_to_hour_'+slot).val()+":"+$wbc('#slot_time_to_minute_'+slot).val()+":"+$wbc('#slot_time_to_ampm_'+slot).val()+"&price="+newprice+"&av="+newav+"&avmax="+newavmax,
			  success: function(data) {
				  if(data == 0) {
					  alert("<?php echo __( 'There\'s another slot for the same date and time. Change values', 'wp-booking-calendar' ); ?>");
				  } else {
			...
			alert("<?php echo __( 'Slot times values are not correct', 'wp-booking-calendar' ); ?>");
		}
	}
	...
	function checkModifyData(frm) {
		...
		with(frm) {
			if(date_from_edit.value == '') {
				alert("<?php echo __( 'Select a date from', 'wp-booking-calendar' ); ?>");
				return false;
			} else if(date_to_edit.value== '' ) {
				alert("<?php echo __( 'Select a date to', 'wp-booking-calendar' ); ?>"); 
				return false;
			} else if(checkEditTimes() == 1) {
				alert("<?php echo __( 'New time range values are not correct', 'wp-booking-calendar' ); ?>");
				return false;
			} else {
				return true;
			}
...
<form name="manage_slots" action="" method="post" style="display:inline;">
	...
	<div id="table" class="booking_margin_t_20">
    	...
    	<div class="booking_border_b_1 booking_border_solid booking_border_ccc booking_bg_f6f booking_height_30 booking_line_30">
			...
			<div class="booking_float_left booking_width_10p"><?php echo __( 'Date', 'wp-booking-calendar' );?>&nbsp;<a href="javascript:orderby('<?php echo $_SESSION["orderbySlotsDate"]; ?>');"><img src="<?php echo plugins_url('wp-booking-calendar/admin/');?>images/orderby_<?php echo $_SESSION["orderbySlotsDate"];?>.gif" border=0 /></a></div>
			
			<div class="booking_float_left booking_width_10p"><?php echo __( 'Time From', 'wp-booking-calendar' );?>&nbsp;<a href="javascript:orderby('<?php echo $_SESSION["orderbySlotsTime"]; ?>');"><img src="<?php echo plugins_url('wp-booking-calendar/admin/');?>images/orderby_<?php echo $_SESSION["orderbySlotsTime"];?>.gif" border=0 /></a></div>
