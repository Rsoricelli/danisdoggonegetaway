	...
	public function addSlot() {
		...
		if(count($resultDate)>0) {
			...
			do {
				...
				$slug_def = $slug;
				//controllo che non esista una slug uguale
				...
				if($holidaysQry==0) {
					for($j=0;$j<count($resultTime);$j++) {
						...
						if($checkQry==0) {
							...
							$wpdb->query($wpdb->prepare("INSERT INTO ".$wpdb->base_prefix.$blog_prefix."booking_slots(slot_special_text,slot_special_mode,slot_date,slot_time_from,slot_time_to,slot_price,slot_av,slot_av_max,slot_active,calendar_id,bundle_id) VALUES(%s,%d,%s,%s,%s,%s,%d,%d,%d,%d,%d)",$slot_special_text,$slot_special_mode,$resultDate[$i],$resultTime[$j]["time_from"],$resultTime[$j]["time_to"],$price,$slot_av,$slot_av_max,1,$_POST["calendar_id"],$bundle_id));
