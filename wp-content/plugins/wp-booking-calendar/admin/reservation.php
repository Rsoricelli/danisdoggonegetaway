...
<div class="booking_padding_20 booking_font_14 booking_line_percent booking_bg_fff">
	...
	<script>
        ...
        function filterReservations() {
            if($wbc('#search_date option:selected').val() == 1) {
                if(Trim($wbc('#first_date').val()) != '') {
                    ...
                    alert("<?php echo __( 'Select a date', 'wp-booking-calendar' );?>");
                }
                ...
                if(Trim($wbc('#first_date').val()) != '' && Trim($wbc('#second_date').val()) != '') {
                    ...
                    alert("<?php echo __( 'Select date range', 'wp-booking-calendar' );?>");
                }
            }
        }
        function delItem(itemId) {
            if(confirm("<?php echo __( 'Are you sure you want to delete this reservation?', 'wp-booking-calendar' );?>")) {
                $wbc.ajax({
            ...
            if($bookingSettingObj->getPaypal()==1 && $bookingSettingObj->getPaypalAccount() != '' && $bookingSettingObj->getPaypalLocale() != '' && $bookingSettingObj->getPaypalCurrency() != '') {
                ...
                $wbc.ajax({
                  ...
                  success: function(data) {
                      if(data>0) {
                          //there's a price
                          if(confirm("<?php echo __( 'Are you sure you want to confirm this reservation? It could not have been paid yet, check your Paypal account before confirmation', 'wp-booking-calendar' );?>")) {
                                $wbc.ajax({
        ...
        function unconfirmReservation(itemId) {
            if(confirm("<?php echo __( 'Are you sure you want to unconfirm this reservation?', 'wp-booking-calendar' );?>")) {
                $wbc.ajax({
		...
		function checkContactData(frm) {
			with(frm) {
				if(Trim(user_contact_email.value) == '' || !isEmailAddr(user_contact_email,true,'')) {
					alert("<?php echo __( 'You must add at least an email address to send the email', 'wp-booking-calendar' );?>");
					return false;
				} else if(Trim(user_contact_subject.value) == '') {
					alert("<?php echo __( 'Insert a subject for the message', 'wp-booking-calendar' ); ?>");
					return false;
				} else if(Trim(user_contact_message.value) == '') {
					alert("<?php echo __( 'Insert a message for the user', 'wp-booking-calendar' ); ?>");
					return false;
				} else {
					return true;
				}
        ...
        <div class="booking_margin_t_20">
            <div id="table" class="booking_font_12">
                ...
                <div class="booking_border_b_1 booking_border_solid booking_border_ccc booking_bg_f6f booking_height_30 booking_line_30">
                    ...
                    <!-- date -->
                    <div class="booking_float_left booking_width_10p"><?php echo __( 'Date', 'wp-booking-calendar' ); ?>&nbsp;<a href="javascript:orderby('date','<?php echo $_SESSION["orderbyReservationDate"]; ?>');"><img src="<?php echo plugins_url('wp-booking-calendar/admin/');?>images/orderby_<?php echo $_SESSION["orderbyReservationDate"];?>.gif" border=0 /></a></div>            
                    
                    <!-- time -->
                    <div class="booking_float_left booking_width_10p"><?php echo __( 'Time', 'wp-booking-calendar' ); ?>&nbsp;<a href="javascript:orderby('time','<?php echo $_SESSION["orderbyReservationTime"]; ?>');"><img src="<?php echo plugins_url('wp-booking-calendar/admin/');?>images/orderby_<?php echo $_SESSION["orderbyReservationTime"];?>.gif" border=0 /></a></div>
                ...
                <div id="row_<?php echo $reservationId; ?>" class="booking_border_b_1 booking_border_solid booking_border_ccc">
                    ...
                    <!-- status -->
                    <div class="booking_float_left booking_width_5p booking_height_50 <?php echo $class; ?>">
                        <div class="booking_wh_inherit booking_table_cell booking_vertical_middle"><span id="conferma_<?php echo $reservationId; ?>"><?php if($reservation["reservation_confirmed"]=='1' && $reservation["reservation_cancelled"] == 0) { ?><a href="javascript:unconfirmReservation(<?php echo $reservationId; ?>);"><img src="<?php echo plugins_url('wp-booking-calendar/admin/');?>images/icons/published.png" border=0 /></a><?php } else if($reservation["reservation_cancelled"] == 0){ ?><a href="javascript:confirmReservation(<?php echo $reservationId; ?>);"><img src="<?php echo plugins_url('wp-booking-calendar/admin/');?>images/icons/unpublished.png" border=0 /></a><?php } else { ?><?php echo __( 'Cancelled', 'wp-booking-calendar' ); ?><?php } ?></span></div>
                    </div>
