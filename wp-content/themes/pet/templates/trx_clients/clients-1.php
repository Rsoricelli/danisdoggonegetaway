...
if ( !function_exists( 'themerex_template_clients_1_output' ) ) {
	function themerex_template_clients_1_output($post_options, $post_data) {
		...
		?>
			<div<?php echo ($post_options['tag_id'] ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''); ?> class="sc_clients_item sc_clients_item_<?php echo esc_attr($post_options['number']) . ($post_options['number'] % 2 == 1 ? ' odd' : ' even') . ($post_options['number'] == 1 ? ' first' : '').(!empty($post_options['tag_class']) ? ' '.esc_attr($post_options['tag_class']) : ''); ?>"<?php echo ($post_options['tag_css'] ? ' style="'.esc_attr($post_options['tag_css']).'"' : '') . (!themerex_sc_param_is_off($post_options['tag_animation']) ? ' data-animation="'.esc_attr(themerex_sc_get_animation_classes($post_options['tag_animation'])).'"' : '');?>>
