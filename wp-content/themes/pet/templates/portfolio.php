...
if ( !function_exists( 'themerex_template_portfolio_theme_setup' ) ) {
	...
	function themerex_template_portfolio_theme_setup() {
		...
		themerex_add_template(array(
			...
			'need_isotope' => true,
			'need_terms' => true,
			'title'  => __('Colored excerpt', 'themerex'),
			'thumb_title'  => __('Small square image (crop)', 'themerex'),
			'w'		 => 300,
			'h'		 => 300
		));
		themerex_add_template(array(
			...
			'need_isotope' => true,
			'need_terms' => true,
			'title'  => __('Colored tile (with hovers, width=height) /2 columns/', 'themerex'),
			'thumb_title'  => __('Large square image (crop)', 'themerex'),
			'w'		 => 870,
			'h' 	 => 870
		));
		themerex_add_template(array(
			...
			'need_isotope' => true,
			'need_terms' => true,
			'title'  => __('Colored tile /3 columns/', 'themerex'),
			'thumb_title'  => __('Medium square image (crop)', 'themerex'),
			'w'		 => 390,
			'h'		 => 390
		));
		themerex_add_template(array(
			...
			'need_isotope' => true,
			'need_terms' => true,
			'title'  => __('Colored tile /4 columns/', 'themerex'),
			'thumb_title'  => __('Small square image (crop)', 'themerex'),
			'w'		 => 300,
			'h'		 => 300
		));
		themerex_add_template(array(
			...
			'need_terms' => true,
			'container_classes' => 'no_margins',
			'title'  => __('Short info /2 columns/', 'themerex'),
			'thumb_title'  => __('Large square image (crop)', 'themerex'),
			'w'		 => 870,
			'h' 	 => 870
		));
		themerex_add_template(array(
			...
			'need_terms' => true,
			'container_classes' => 'no_margins',
			'title'  => __('Short info /3 columns/', 'themerex'),
			'thumb_title'  => __('Medium square image (crop)', 'themerex'),
			'w'		 => 390,
			'h'		 => 390
		));
		themerex_add_template(array(
			...
			'need_terms' => true,
			'container_classes' => 'no_margins',
			'title'  => __('Short info /4 columns/', 'themerex'),
			'thumb_title'  => __('Small square image (crop)', 'themerex'),
			'w'		 => 300,
			'h'		 => 300
		));
						...
						<div class="post_info_wrap info"><div class="info-back">
							...
							<?php
								if ($post_data['post_protected']) {
										...
										echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
											...
											: '<p>' . ($link_start)
												. (themerex_strpos($post_options['hover'], 'square')!==false
                                                    ? ( $post_options['layout'] == 'portfolio_3' ? trim(themerex_strshort($post_data['post_excerpt'], isset($post_options['descr']) ? '200' : '200')) .'... ' : strip_tags($post_data['post_excerpt']) )
									...
									if ( $post_data['post_link'] != '' &&  !in_array($post_options['layout'] , array('portfolio_3', 'portfolio_2', 'portfolio_4')) ) {
										?><p class="post_buttons"><?php
										if ($style=='colored_1') {
											?><span class="post_button"><?php echo do_shortcode('[trx_button size="small" link="'.esc_url($post_data['post_link']).'"]'.__('Learn more', 'themerex').'[/trx_button]'); ?></span><?php
										} else if (!themerex_sc_param_is_off($post_options['readmore']) && !in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status'))) {
