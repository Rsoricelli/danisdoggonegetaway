...
if ( !function_exists( 'themerex_template_plain_theme_setup' ) ) {
	...
	function themerex_template_plain_theme_setup() {
		themerex_add_template(array(
			...
			'need_terms' => true,
			'mode'   => 'blogger',
			'title'  => __('Blogger layout: Plain', 'themerex')
			));
	}
}
...
if ( !function_exists( 'themerex_template_plain_output' ) ) {
	function themerex_template_plain_output($post_options, $post_data) {
		?>
		<div class="post_item sc_blogger_item sc_plain_item<?php echo ($post_options['number'] == $post_options['posts_on_page'] && !themerex_sc_param_is_on($post_options['loadmore']) ? ' sc_blogger_item_last' : ''); ?>">
			
			<?php
			if (!empty($post_data['post_terms'][$post_data['post_taxonomy']]->terms_links)) {
				?>
				<div class="post_category">
					<span class="post_category_label"><?php _e('in', 'themerex'); ?></span> <?php echo join(', ', $post_data['post_terms'][$post_data['post_taxonomy']]->terms_links); ?>
				</div>
				<?php
			}
