...
if ( !function_exists( 'themerex_template_services_1_theme_setup' ) ) {
	...
	function themerex_template_services_1_theme_setup() {
		themerex_add_template(array(
			...
			'template' => 'services-1',
			'mode'   => 'services',
			'title'  => __('Services /Style 1/', 'themerex'),
		...
		?>
			<div<?php echo ($post_options['tag_id'] ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''); ?>
				...
				<?php echo ($post_options['tag_css']!='' ? ' style="'.esc_attr($post_options['tag_css']).'"' : '') 
					. (!themerex_sc_param_is_off($post_options['tag_animation']) ? ' data-animation="'.esc_attr(themerex_sc_get_animation_classes($post_options['tag_animation'])).'"' : ''); ?>>
				<?php 
				if ($post_data['post_icon'] && $post_options['tag_type']=='icons') {
					$html = do_shortcode('[trx_icon icon="'.esc_attr($post_data['post_icon']).'" shape="round"]');
				...
				<div class="sc_services_item_content">
					...
					<div class="sc_services_item_description">
						...
						if ($post_data['post_protected']) {
							...
							if (!empty($post_data['post_link']) && !themerex_sc_param_is_off($post_options['readmore'])) {
