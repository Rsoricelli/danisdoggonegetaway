...
if ( !function_exists( 'themerex_template_header_4_theme_setup' ) ) {
	add_action( 'themerex_action_before_init_theme', 'themerex_template_header_4_theme_setup', 1 );
	function themerex_template_header_4_theme_setup() {
		themerex_add_template(array(
			'layout' => 'header_4',
			'mode'   => 'header',
			'title'  => __('Header 4', 'themerex'),
			'icon'   => themerex_get_file_url('templates/headers/images/4.jpg')
			));
	}
}

// Template output
if ( !function_exists( 'themerex_template_header_4_output' ) ) {
	function themerex_template_header_4_output($post_options, $post_data) {
		...
		<header class="top_panel_wrap top_panel_style_4 scheme_<?php echo esc_attr($post_options['scheme']); ?>">
			<div class="top_panel_wrap_inner top_panel_inner_style_4 top_panel_position_<?php echo esc_attr(themerex_get_custom_option('top_panel_position')); ?>">
				...
				<div class="content_wrap">
					<div class="columns_wrap columns_fluid"><div
						...
						class="column-2_3 menu_main_wrap">
							...
							<?php if (themerex_get_custom_option('show_search')=='yes') echo do_shortcode('[trx_search]'); ?>
						</div>
					</div>
				</div>
			</div>
