...
<body <?php 
	body_class('themerex_body body_style_' . esc_attr($body_style) 
		...
		. (!themerex_sc_param_is_off($top_panel_position) ? ' top_panel_show top_panel_' . esc_attr($top_panel_position) : 'top_panel_hide')
		. ' ' . esc_attr(themerex_get_sidebar_class())
		. ($video_bg_show ? ' video_bg_show' : '')
        . (themerex_get_custom_option('style_404')=='color' ? ' color_404 ' : '')
    );
	?>
>
	...
	if (themerex_get_custom_option('menu_toc_home')=='yes') echo do_shortcode( '[trx_anchor id="toc_home" title="'.__('Home', 'themerex').'" description="'.__('{{Return to Home}} - ||navigate to home page of the site', 'themerex').'" icon="icon-home" separator="yes" url="'.home_url().'"]' ); 
	if (themerex_get_custom_option('menu_toc_top')=='yes') echo do_shortcode( '[trx_anchor id="toc_top" title="'.__('To Top', 'themerex').'" description="'.__('{{Back to top}} - ||scroll to top of the page', 'themerex').'" icon="icon-double-up" separator="yes"]' ); 
	?>
