...
function themerex_sc_accordion($atts, $content=null){	
	...
	$THEMEREX_GLOBALS['sc_accordion_icon_closed'] = empty($icon_closed) || themerex_sc_param_is_inherit($icon_closed) ? "icon-down" : $icon_closed;
	$THEMEREX_GLOBALS['sc_accordion_icon_opened'] = empty($icon_opened) || themerex_sc_param_is_inherit($icon_opened) ? "icon-up" : $icon_opened;
...
function themerex_sc_accordion_item($atts, $content=null)
{
    ...
    if (empty($icon_closed) || themerex_sc_param_is_inherit($icon_closed)) $icon_closed = $THEMEREX_GLOBALS['sc_accordion_icon_closed'] ? $THEMEREX_GLOBALS['sc_accordion_icon_closed'] : "icon-up";
    if (empty($icon_opened) || themerex_sc_param_is_inherit($icon_opened)) $icon_opened = $THEMEREX_GLOBALS['sc_accordion_icon_opened'] ? $THEMEREX_GLOBALS['sc_accordion_icon_opened'] : "icon-down";
    ...
    $output = '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '')
        ...
        . ($title)
            . '<span class="second-letter-accordion">'
...
function themerex_sc_audio($atts, $content = null) {
	...
	if ( themerex_get_custom_option('substitute_audio')=='no') {
		if (themerex_sc_param_is_on($frame)) {
...
function themerex_sc_call_to_action($atts, $content=null){	
	...
	$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
			. ' class="sc_call_to_action'
				. (themerex_sc_param_is_on($accent) ? ' sc_call_to_action_accented' : '')
			...
			. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
		. '>'
			. (themerex_sc_param_is_on($accent) ? '<div class="content_wrap">' : '')
			...
			. ($align=='right' ? $featured : '')
			. ($need_columns ? '</div>' : '')
			. (themerex_sc_param_is_on($accent) ? '</div>' : '')
		. '</div>';
...
function themerex_sc_contact_form($atts, $content = null) {
	...
	$output = '<div ' . ($id ? ' id="'.esc_attr($id).'"' : '') 
			. ' class="sc_contact_form'
				. ' sc_contact_form_'.($content != '' && themerex_sc_param_is_on($custom) ? 'custom' : 'standard') 
	...
	$output .= '<form' . ($id ? ' id="'.esc_attr($id).'"' : '') . ' data-formtype="'.($content ? 'custom' : 'contact').'" method="post" action="' . esc_url($action ? $action : $THEMEREX_GLOBALS['ajax_url']) . '">'
				. ($content != '' && themerex_sc_param_is_on($custom)
					...
					: '<div class="sc_contact_form_info">'
                            ...
                            . '<div class="sc_contact_form_item sc_contact_form_field label_over"><label class="required" for="sc_contact_form_phone">' . __('Phone', 'themerex') . '</label><input id="sc_contact_form_phone" type="text" name="phone" placeholder="' . __('Phone', 'themerex') . '"></div>'
...
function themerex_sc_list($atts, $content=null){	
	...
	$THEMEREX_GLOBALS['sc_list_icon'] = empty($icon) || themerex_sc_param_is_inherit($icon) ? "icon-right" : $icon;
...
function themerex_sc_list_item($atts, $content=null) {
	...
	$output = '<li' . ($id ? ' id="'.esc_attr($id).'"' : '') 
		...
		. ($title ? ' title="'.esc_attr($title).'"' : '') 
		. '>' 
		. (!empty($link) ? '<a href="'.esc_url($link).'"' . (!empty($target) ? ' target="'.esc_attr($target).'"' : '') . (' class="trx_list_color"') .'>' : '<span class="trx_list_color">')
...
function themerex_sc_price_block($atts, $content=null){	
	...
	$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
				...
				. (!themerex_sc_param_is_off($animation) ? ' data-animation="'.esc_attr(themerex_sc_get_animation_classes($animation)).'"' : '')
				. '>'
			. (!empty($title) ? '<div class="sc_price_block_title">'.($title).'
            ...
            . '<div class="sc_price_block_money">'
                ...
                . ($money)
            . '</div>'
			. (!empty($link_text) ? '<div class="sc_price_block_link">'.do_shortcode('[trx_button bg_color="' .esc_attr($color). '" icon="icon-right"   link="'.($link ? esc_url($link) : '#').'"]'.($link_text).'[/trx_button]').'</div>' : '')
		. '</div>';
				...
				. '>
					<div class="search_form_wrap">
						<form role="search" method="get" class="search_form" action="' . esc_url( home_url( '/' ) ) . '">
							<button type="submit" class="search_submit icon-search-light" title="' . ($state=='closed' ? __('Open search', 'themerex') : __('Start search', 'themerex')) . '">' . __('Search', 'themerex') . '</button>
					...
					<div class="search_results widget_area' . ($scheme && !themerex_sc_param_is_off($scheme) && !themerex_sc_param_is_inherit($scheme) ? ' scheme_'.esc_attr($scheme) : '') . '"><a class="search_results_close icon-cancel"></a><div class="search_results_content"></div></div>
			</div>';
...
function themerex_sc_section($atts, $content=null){	
	...
	$css .= themerex_get_css_position_from_values('!'.($top), '!'.($right), '!'.($bottom), '!'.($left))
		...
		.($font_weight != '' && !themerex_sc_param_is_inherit($font_weight) ? 'font-weight:' . esc_attr($font_weight) . ';' : '');
				...
				. ($type=='pie' && themerex_sc_param_is_on($pie_compact)
					? ('<div class="sc_skills_legend">'.($THEMEREX_GLOBALS['sc_skills_legend']).'</div>'
						. '<div id="'.esc_attr($id).'_pie" class="sc_skills_item">'
                            ...
                            . '<div class="max_skill_value_pie">' . esc_attr($THEMEREX_GLOBALS['sc_skills_max']) . '</div>'
							. '<div class="sc_skills_data" style="display:none;">' . ($THEMEREX_GLOBALS['sc_skills_data']) . '</div>'
                . '</div>'
					  )
					: '')
...
//  Colors from gradient
    $color1 = themerex_get_scheme_color('accent1');
    $color2 = themerex_get_scheme_color('accent2');
    $color3 = themerex_get_scheme_color('accent3');

	if (empty($color)) $color = $THEMEREX_GLOBALS['sc_skills_color'];
//  Old version
//	if (empty($color)) $color = themerex_get_scheme_color('accent2', $color);
	if (empty($color) && $THEMEREX_GLOBALS['sc_skills_type'] != 'bar') $color = themerex_get_scheme_color('accent1', $color);
    if (empty($color) && $THEMEREX_GLOBALS['sc_skills_type'] == 'bar') $color = 1;
	...
	if ($THEMEREX_GLOBALS['sc_skills_type'] == 'arc' || ($THEMEREX_GLOBALS['sc_skills_type'] == 'pie' && themerex_sc_param_is_on($THEMEREX_GLOBALS['sc_skills_pie_compact']))) {
		...
		if ($THEMEREX_GLOBALS['sc_skills_type'] == 'bar') {
            $output .= '<div class="sc_skills_total"'
                ...
                . ($start) . ($ed)
                . '</div>';
            $output .= '<div class="sc_skills_count"' . ($THEMEREX_GLOBALS['sc_skills_type'] == 'bar' && $color == 1 ? ' style="background: linear-gradient(to right,' . esc_attr($color3) .' 0%, '. esc_attr($color1) .' 30%,'. esc_attr($color2) .' 70%, rgb(100, 140, 160) 100%)' .  '; border-color:' . esc_attr($color) . '"' : ' style="background-color:' . esc_attr($color) . '; border-color:' . esc_attr($color) . '"') . '>'
//          Old version
//          $output .= '<div class="sc_skills_count"' . ($THEMEREX_GLOBALS['sc_skills_type'] == 'bar' && $color ? ' style="background-color:' . esc_attr($color) . '; border-color:' . esc_attr($color) . '"' : '') . '>'
                . '</div>';
        } else if ($THEMEREX_GLOBALS['sc_skills_type']=='counter') {
            $output .= '<div class="sc_skills_count"' . ($THEMEREX_GLOBALS['sc_skills_type']=='bar' && $color ? ' style="background-color:' . esc_attr($color) . '; border-color:' . esc_attr($color) . '"' : '') . '>'
...
function themerex_sc_tabs($atts, $content = null) {
	...
	for ($i = 0; $i < count($sc_tab_titles); $i++) {
		...
		$titles_output .= '<li class="'.join(' ', $classes).'">'
                            ...
                            . '<span class="sc_tabs_icon sc_tabs_icon_before_title ' . ($sc_tab_titles[$i]['icon_before_title']) . '"></span>'
...
function themerex_sc_tab($atts, $content = null) {
	...
	if (isset($sc_tab_titles[$THEMEREX_GLOBALS['sc_tab_counter']-1])) {
		...
		$sc_tab_titles[] = array(
			'id' => $id,
			'title' => $title,
            'icon_before_title' => $icon_before_title
		);
	}
...
function themerex_sc_title($atts, $content=null){	
	...
	$css .= themerex_get_css_position_from_values($top, $right, $bottom, $left, $width)
		.($align && $align!='none' && !themerex_sc_param_is_inherit($align) ? 'text-align:' . esc_attr($align) .';' : '')
		.($color ? 'color:' . esc_attr($color) .';' : '')
		.($font_weight && !themerex_sc_param_is_inherit($font_weight) ? 'font-weight:' . esc_attr($font_weight) .';' : '')
	...
	$output = '<h' . esc_attr($type) . ($id ? ' id="'.esc_attr($id).'"' : '')
			. ' class="sc_title sc_title_'.esc_attr($style)
				.($align && $align!='none' && !themerex_sc_param_is_inherit($align) ? ' sc_align_' . esc_attr($align) : '')
				.(!empty($class) ? ' '.esc_attr($class) : '')
				.'"'
			. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
			. (!themerex_sc_param_is_off($animation) ? ' data-animation="'.esc_attr(themerex_sc_get_animation_classes($animation)).'"' : '')
			. '>'
                ...
                . ($style=='underline'  ? '<span class="tile_underline"><span class="tile_underline_semicircle semicircle_1"></span><span class="tile_underline_semicircle semicircle_2"></span><span class="tile_underline_semicircle semicircle_3"></span></span>' : '')
...
function themerex_sc_toggles($atts, $content=null){	
	...
	$THEMEREX_GLOBALS['sc_toggles_icon_closed'] = empty($icon_closed) || themerex_sc_param_is_inherit($icon_closed) ? "icon-plus" : $icon_closed;
	$THEMEREX_GLOBALS['sc_toggles_icon_opened'] = empty($icon_opened) || themerex_sc_param_is_inherit($icon_opened) ? "icon-minus" : $icon_opened;
...
function themerex_sc_toggles_item($atts, $content=null) {
	...
	if (empty($icon_closed) || themerex_sc_param_is_inherit($icon_closed)) $icon_closed = $THEMEREX_GLOBALS['sc_toggles_icon_closed'] ? $THEMEREX_GLOBALS['sc_toggles_icon_closed'] : "icon-plus";
	if (empty($icon_opened) || themerex_sc_param_is_inherit($icon_opened)) $icon_opened = $THEMEREX_GLOBALS['sc_toggles_icon_opened'] ? $THEMEREX_GLOBALS['sc_toggles_icon_opened'] : "icon-minus";
		...
		if ($data && isset($data[0]['text'])) {
			...
			$output = ($bg_color!='' || $bg_image!='' || $bg_overlay>0 || $bg_texture>0 || themerex_strlen($bg_texture)>2 || ($scheme && !themerex_sc_param_is_off($scheme) && !themerex_sc_param_is_inherit($scheme))
					? '<div class="sc_twitter_wrap sc_section'
							. ($scheme && !themerex_sc_param_is_off($scheme) && !themerex_sc_param_is_inherit($scheme) ? ' scheme_'.esc_attr($scheme) : '') 
							. ($align && $align!='none' && !themerex_sc_param_is_inherit($align) ? ' align' . esc_attr($align) : '')
							. '"'
						.' style="'
							. ($bg_color !== '' && $bg_overlay==0 ? 'background-color:' . esc_attr($bg_color) . ';' : '')
							. ($bg_image !== '' ? 'background-image:url('.esc_url($bg_image).');' : '')
					...
					. '<div class="sc_twitter sc_slider_swiper sc_slider_nopagination swiper-slider-container"'
							...
							. ($hs ? ' sc_slider_height_fixed' : '')
							. (!empty($class) ? ' '.esc_attr($class) : '')
							. ($bg_color=='' && $bg_image=='' && $bg_overlay==0 && ($bg_texture=='' || $bg_texture=='0') && $align && $align!='none' && !themerex_sc_param_is_inherit($align) ? ' align' . esc_attr($align) : '')
							. '"'
