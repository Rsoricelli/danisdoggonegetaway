...
function themerex_shortcodes_show_field(field, tab_idx) {
	...
	for (var clone_num in field['params']) {
		...
		for (var param_num in field['params'][clone_num]) {
			...
			if (param['type']=='media') {
				if (!themerex_isset(param['before'])) param['before'] = {};
				param['before'] = themerex_merge_objects({
						'title': 'Choose image',
						'action': 'media_upload',
						'type': 'image',
						'multiple': false,
				...
				if (!themerex_isset(param['after'])) param['after'] = {};
				param['after'] = themerex_merge_objects({
						'icon': 'iconadmin-cancel',
						'action': 'media_reset'
					}, param['after']);
			}
			if (param['type']=='color' && (THEMEREX_GLOBALS['shortcodes_cp']=='tiny' || (themerex_isset(param['style']) && param['style']!='wp'))) {
				if (!themerex_isset(param['after'])) param['after'] = {};
				param['after'] = themerex_merge_objects({
						'icon': 'iconadmin-cancel',
						'action': 'color_reset'
			...
			case 'socials':
				...
				if (themerex_empty(param['style']) || param['style']=='icons') {
					rez2 = themerex_shortcodes_action_button({
						...
						'icon': (themerex_empty(param['style']) || param['style']=='icons') && !themerex_empty(param['value']['icon']) ? param['value']['icon'] : 'iconadmin-users'
						}, 'after');
				} else
					rez2 = ['', ''];
			...
			case "color":
				var cp_style = themerex_isset(param['style']) ? param['style'] : THEMEREX_GLOBALS['shortcodes_cp'];
				output += '<input class="themerex_options_input themerex_options_input_color themerex_options_input_color_'+cp_style +'"'
					+ ' name="' + id + '"'
					+ ' id="' + id + '"'
					+ ' data-param="' + themerex_shortcodes_prepare_value(param_num) + '"'
					+ ' type="text"'
					...
					+ before;
				if (cp_style=='custom')
					output += '<span class="themerex_options_input_colorpicker iColorPicker"></span>';
...
function themerex_shortcodes_action_button(data, type) {
	...
	var output = '<span class="' 
				...
				+ (!themerex_empty(data['sizes']) ? ' data-sizes="'+data['sizes']+'"' : '')
