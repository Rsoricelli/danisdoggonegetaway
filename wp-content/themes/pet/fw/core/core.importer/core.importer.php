...
class themerex_dummy_data_importer {
	...
	var $options = array(
		...
		'domain_dev'			=> 'pet.dv.ancorathemes.com',	// Domain on developer's server
		'domain_demo'			=> 'pet.ancorathemes.com',		// Domain on demo-server
		'demo_url'				=> 'http://pet.ancorathemes.com/',	// URL of the demo site - need for change URLs in the custom menu items
		...
		'page_for_posts'		=> 'Blog post width sidebar',	// Blog streampage title
...
if ($this->options['debug']) { printf(__('Can not locate Essential Grid plugin: %s', 'themerex'), EG_PLUGIN_PATH.'/essential-grid.php<br>'); flush(); }
	...
	function replace_uploads($str) {
		...
		return $str;
		*/
		return themerex_replace_uploads_url($str, $this->options['uploads_folder']);
	}
