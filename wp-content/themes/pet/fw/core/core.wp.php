...
if (!function_exists('themerex_wp_theme_setup')) {
	...
	function themerex_wp_theme_setup() {
		...
		add_action('publish_post',							'themerex_init_post_counters');
		add_action('save_post',								'themerex_init_post_counters');
	...
	function themerex_get_calendar($onlyFirstLetter = false, $get_month = 0, $get_year = 0, $opt=array()) {
		...
		if (is_array($myweek) && count($myweek) > 0) {
			foreach ($myweek as $wd) {
				$day_name = $onlyFirstLetter ?  $wp_locale->get_weekday_abbrev($wd) : $wp_locale->get_weekday_initial($wd);
				$wd = esc_attr($wd);
				...
				<li id="comment-<?php comment_ID(); ?>" <?php comment_class('comment_item'); ?>>
					...
					<div class="comment_content">
						<div class="comment_info">
							...
							<span class="comment_date"><span class="comment_date_label"></span> <span class="comment_date_value"><?php echo get_comment_date(get_option('date_format')); ?></span></span>
...
if (!function_exists('themerex_get_post_data')) {
	function themerex_get_post_data(&$opt, $post_obj=null) {
		...
		if (!empty($post_content) && $opt['content']) {
			if ($opt['substitute_gallery'] && !themerex_sc_in_shortcode_blogger(true))	$post_content = themerex_substitute_gallery($post_content, $post_id, $thumb_sizes['w'], $thumb_sizes['h_crop'], 'none', true);
			$post_content = apply_filters('the_content', $post_content);
			if ($post_id != get_the_ID()) {		// Fix bug in the WPML
				$post = $cur_post;
				setup_postdata($post);
			}
			if ($opt['substitute_video'] && !themerex_sc_in_shortcode_blogger(true)) 	$post_content = themerex_substitute_video($post_content, $thumb_sizes['w'], $thumb_sizes['h_crop']);
			if ($opt['substitute_audio'] && !themerex_sc_in_shortcode_blogger(true))	$post_content = themerex_substitute_audio($post_content);
		}
		...
		if (!empty($post_excerpt)) {
			if ($opt['substitute_gallery'] && !themerex_sc_in_shortcode_blogger(true)) $post_excerpt = themerex_substitute_gallery($post_excerpt, $post_id, $thumb_sizes['w'], $thumb_sizes['h_crop']);
			...
			if ($opt['substitute_video'] && !themerex_sc_in_shortcode_blogger(true)) $post_excerpt = themerex_substitute_video($post_excerpt, $thumb_sizes['w'], $thumb_sizes['h_crop']);
			if ($opt['substitute_audio'] && !themerex_sc_in_shortcode_blogger(true)) $post_excerpt = themerex_substitute_audio($post_excerpt);
		...
		if ($opt['substitute_video'] && !themerex_sc_in_shortcode_blogger(true)) {
		...
		if ($opt['substitute_audio'] && !themerex_sc_in_shortcode_blogger(true)) {
		...
		if ($post_format == 'gallery') {
			...
			if ($post_video!='' && $opt['substitute_video'] && !themerex_sc_in_shortcode_blogger(true)) {
			...
			if ($post_audio!='' && $opt['substitute_audio'] && !themerex_sc_in_shortcode_blogger(true)) {
...
// add_action('save_post',		'themerex_init_post_counters');
// add_action('publish_post',	'themerex_init_post_counters');
if ( !function_exists( 'themerex_init_post_counters' ) ) {
	function themerex_init_post_counters($id) {
		themerex_get_post_views($id);
		themerex_get_post_likes($id);
	}
}
...
// Moved into plugin
if (false && !function_exists('themerex_require_data')) {	
