...
if ( !function_exists( 'themerex_options_admin_menu_item' ) ) {
	...
	function themerex_options_admin_menu_item() {
		...
		add_menu_page(__('Global Options', 'themerex'), __('Theme Options', 'themerex'), 'manage_options', 'themerex_options', 'themerex_options_page');
		add_submenu_page('themerex_options', __('Global Options', 'themerex'), __('Global Options', 'themerex'), 'manage_options', 'themerex_options', 'themerex_options_page');
		...
		if (!empty($inheritance) && is_array($inheritance)) {
			foreach($inheritance as $k=>$v) {
				...
				if (!empty($v['stream_template'])) {
					...
					add_submenu_page('themerex_options', $title.' '.__('Options', 'themerex'), $title, 'manage_options', 'themerex_options_'.($slug), 'themerex_options_page');
					$tpl = true;
				}
...
// Get theme setting: not editable settings
if ( !function_exists( 'themerex_get_theme_setting' ) ) {
	function themerex_get_theme_setting($option_name) {
		global $THEMEREX_GLOBALS;
		return $THEMEREX_GLOBALS['settings'][$option_name];
	}
}
	...
	function themerex_options_prepare_scripts($override='') {
		...
		<script type="text/javascript">
			jQuery(document).ready(function () {
				...
				try {
					THEMEREX_GLOBALS['to_options']	= eval(<?php echo json_encode( themerex_array_prepare_to_json($THEMEREX_GLOBALS['to_data']) ); ?>);
				} catch(e) {}
				...
				THEMEREX_GLOBALS['to_export_list']	= [<?php
					if (($export_opts = get_option('themerex_options_export_'.($override), false)) !== false) {
						$keys = join('","', array_keys($export_opts));
						if ($keys) echo '"'.($keys).'"';
					}
				?>];
				if (THEMEREX_GLOBALS['to_strings']==undefined) THEMEREX_GLOBALS['to_strings'] = {};
				THEMEREX_GLOBALS['to_strings'].del_item_error			= "<?php _e("You can't delete last item! To disable it - just clear value in field.", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].del_item 				= "<?php _e("Delete item error!", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].wait 					= "<?php _e("Please wait a few seconds!", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].save_options				= "<?php _e("Options saved!", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].reset_options			= "<?php _e("Options reset!", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].reset_options_confirm	= "<?php _e("Do you really want reset all options to default values?", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].reset_options_complete	= "<?php _e("Settings are reset to their default values. The page will automatically reload in 3 sec.", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].export_options_header 	= "<?php _e("Export options", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].export_options_error		= "<?php _e("Name for options set is not selected! Export cancelled.", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].export_options_label		= "<?php _e("Name for the options set:", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].export_options_label2	= "<?php _e("or select one of exists set (for replace):", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].export_options_select	= "<?php _e("Select set for replace ...", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].export_empty				= "<?php _e("No exported sets for import!", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].export_options			= "<?php _e("Options exported!", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].export_link				= "<?php _e("If need, you can download the configuration file from the following link: %s", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].export_download			= "<?php _e("Download theme options settings", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].import_options_label		= "<?php _e("or put here previously exported data:", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].import_options_label2	= "<?php _e("or select file with saved settings:", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].import_options_header	= "<?php _e("Import options", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].import_options_error		= "<?php _e("You need select the name for options set or paste import data! Import cancelled.", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].import_options_failed	= "<?php _e("Error while import options! Import cancelled.", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].import_options_broken	= "<?php _e("Attention! Some options are not imported:", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].import_options			= "<?php _e("Options imported!", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].import_dummy_confirm		= "<?php _e("Attention! During the import process, all existing data will be replaced with new.", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].clear_cache				= "<?php _e("Cache cleared successfull!", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].clear_cache_header		= "<?php _e("Clear cache", 'themerex'); ?>";
			});
		</script>
		<?php 
	}
...
if ( !function_exists( 'themerex_options_page' ) ) {
	function themerex_options_page() {
		...
		themerex_options_page_start(array(
			...
			'subtitle' => empty($slug) 
								? (empty($override) 
									? __('Global Options', 'themerex')
									: '') 
								: themerex_strtoproper(str_replace('_', ' ', $slug)) . ' ' . __('Options', 'themerex'),
			'description' => empty($slug) 
								? (empty($override) 
									? __('Global settings affect the entire website\'s display. They can be overriden when editing pages/categories/posts', 'themerex')
									: '') 
								: __('Settings template for a certain post type: affects the display of just one specific post type. They can be overriden when editing categories and/or posts of a certain type', 'themerex'),
			'slug' => $slug,
			'override' => $override
		));
...
if ( !function_exists( 'themerex_options_page_start' ) ) {
	function themerex_options_page_start($args = array()) {
		$to_flags = array_merge(array(
			'data'				=> null,
			'title'				=> __('Theme Options', 'themerex'),	// Theme Options page title
					...
					echo '<label class="themerex_options_field_label'
						...
						. (!empty($THEMEREX_GLOBALS['to_flags']['add_inherit']) && isset($field['std']) 
							? '<span id="'.esc_attr($id).'_inherit" class="themerex_options_button_inherit'
								...
								.'" title="' . __('Unlock this field', 'themerex') . '"></span>' 
							: '')
						. '</label>'
						. "\n";
				}
...
if ( !function_exists( 'themerex_options_import' ) ) {
	...
	function themerex_options_import() {
		...
		if (!empty($text)) {
			...
			$import = get_option('themerex_options_export_'.($override), array());
