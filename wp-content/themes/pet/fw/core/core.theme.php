...
if ( !function_exists( 'themerex_register_theme_menus' ) ) {
	function themerex_register_theme_menus() {
		register_nav_menus(apply_filters('themerex_filter_add_theme_menus', array(
			...
			'menu_footer'	=> __('Footer Menu', 'themerex'),
...
if ( !function_exists( 'themerex_core_frontend_scripts' ) ) {
	function themerex_core_frontend_scripts() {
        ...
        $fonts_subset = 'latin,latin-ext'; //latin,latin-ext,greek,greek-ext,cyrillic,cyrillic-ext,vietnamese
		...
		if (is_array($theme_fonts) && count($theme_fonts) > 0) {
            ...
            if ($google_fonts) {
                themerex_enqueue_style( 'themerex-font-google_fonts-style', themerex_get_protocol() . '://fonts.googleapis.com/css?family=' . $google_fonts . '&subset=' . $fonts_subset, array(), null );/***/
            }
		}
...
/*+*/$THEMEREX_GLOBALS['current_sidebar'] = 'left_menu';
// Add class "widget_number_#' for each widget
