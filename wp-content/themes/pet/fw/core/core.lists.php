...
if ( !function_exists( 'themerex_get_list_styles' ) ) {
    function themerex_get_list_styles($from=1, $to=2, $prepend_inherit=false) {
        $list = array();
        for ($i=$from; $i<=$to; $i++)
            $list[$i] = sprintf(__('Style %d', 'themerex'), $i);
...
if ( !function_exists( 'themerex_get_list_slider_controls' ) ) {
	function themerex_get_list_slider_controls($prepend_inherit=false) {
		...
		if (isset($THEMEREX_GLOBALS['list_slider_controls']))
			...
			$list = array(
				'no' => __('None', 'themerex'),
				'side' => __('Side', 'themerex'),
				'bottom' => __('Bottom', 'themerex'),
				'pagination' => __('Pagination', 'themerex')
			);
...
if ( !function_exists( 'themerex_get_slider_controls_classes' ) ) {
	function themerex_get_slider_controls_classes($controls) {
		if (themerex_sc_param_is_off($controls))	$classes = 'sc_slider_nopagination sc_slider_nocontrols';
...
if ( !function_exists( 'themerex_get_sidebar_class' ) ) {
	function themerex_get_sidebar_class() {
		...
		return (themerex_sc_param_is_off($sb_main) ? 'sidebar_hide' : 'sidebar_show sidebar_'.($sb_main))
				. ' ' . (themerex_sc_param_is_off($sb_outer) ? 'sidebar_outer_hide' : 'sidebar_outer_show sidebar_outer_'.($sb_outer));
	}
}
...
if ( !function_exists( 'themerex_get_list_hovers' ) ) {
	...
	//		$list['square effect4']  = __('Square Effect 4',  'themerex');
			...
			$list['square effect_more']  = __('Square Effect More',  'themerex');
			$list['square effect_fade']  = __('Square Effect Fade',  'themerex');
...
if ( !function_exists( 'themerex_get_list_bg_image_positions' ) ) {
	function themerex_get_list_bg_image_positions($prepend_inherit=false) {
		...
		if (isset($THEMEREX_GLOBALS['list_bg_image_positions']))
			...
			$list = array();
			$list['left top']	  = __('Left Top', 'themerex');
			$list['center top']   = __("Center Top", 'themerex');
			$list['right top']    = __("Right Top", 'themerex');
			$list['left center']  = __("Left Center", 'themerex');
			$list['center center']= __("Center Center", 'themerex');
			$list['right center'] = __("Right Center", 'themerex');
			$list['left bottom']  = __("Left Bottom", 'themerex');
			$list['center bottom']= __("Center Bottom", 'themerex');
			$list['right bottom'] = __("Right Bottom", 'themerex');
...
if ( !function_exists( 'themerex_get_list_bg_image_repeats' ) ) {
	function themerex_get_list_bg_image_repeats($prepend_inherit=false) {
		...
		if (isset($THEMEREX_GLOBALS['list_bg_image_repeats']))
			...
			$list = array();
			$list['repeat']	  = __('Repeat', 'themerex');
			$list['repeat-x'] = __('Repeat X', 'themerex');
			$list['repeat-y'] = __('Repeat Y', 'themerex');
			$list['no-repeat']= __('No Repeat', 'themerex');
...
if ( !function_exists( 'themerex_get_list_bg_image_attachments' ) ) {
	function themerex_get_list_bg_image_attachments($prepend_inherit=false) {
		...
		if (isset($THEMEREX_GLOBALS['list_bg_image_attachments']))
			...
			$list = array();
			$list['scroll']	= __('Scroll', 'themerex');
			$list['fixed']	= __('Fixed', 'themerex');
			$list['local']	= __('Local', 'themerex');
...
if ( !function_exists( 'themerex_get_list_googlemap_styles' ) ) {
	function themerex_get_list_googlemap_styles($prepend_inherit=false) {
		...
		if (isset($THEMEREX_GLOBALS['list_googlemap_styles']))
			...
			$list['blue_grey'] = __('Blue and Grey', 'themerex');
...
if ( !function_exists( 'themerex_get_list_sizes' ) ) {
	function themerex_get_list_sizes($prepend_inherit=false) {
		...
		if (isset($THEMEREX_GLOBALS['list_sizes']))
			...
			$list = array();
			$list["tiny"]   = __("Tiny", 'themerex');
			$list["small"]  = __("Small", 'themerex');
			$list["medium"] = __("Medium", 'themerex');
			$list["large"]  = __("Large", 'themerex');
