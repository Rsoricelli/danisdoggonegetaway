...
if ( !function_exists( 'themerex_core_customizer_theme_setup' ) ) {
	...
	function themerex_core_customizer_theme_setup() {
		...
		add_action('themerex_action_add_scripts_inline',			'themerex_core_customizer_add_scripts_inline');
		...
		if ( is_admin() ) {
			...
			add_action('wp_ajax_nopriv_themerex_options_save',			'themerex_core_customizer_save_options');
			...
			add_action('wp_ajax_nopriv_themerex_options_scheme_delete',	'themerex_core_customizer_scheme_delete');
			...
			add_action('wp_ajax_nopriv_themerex_options_scheme_copy',	'themerex_core_customizer_scheme_copy');
		}
...
if ( !function_exists( 'themerex_core_customizer_admin_menu_item' ) ) {
	...
	function themerex_core_customizer_admin_menu_item() {
		add_submenu_page('themerex_options', __('Fonts & Colors', 'themerex'), __('Fonts & Colors', 'themerex'), 'manage_options', 'themerex_options_customizer', 'themerex_core_customizer_page');
	}
}
...
if ( !function_exists( 'themerex_core_customizer_scheme_delete' ) ) {
	...
	function themerex_core_customizer_scheme_delete() {
		...
		global $THEMEREX_GLOBALS;
		$scheme = $_POST['scheme'];
		$order = !empty($_POST['order']) ? explode(',', $_POST['order']) : array_keys($THEMEREX_GLOBALS['custom_colors']);
		...
		if (isset($THEMEREX_GLOBALS['custom_colors'][$scheme])) {
			if (count($THEMEREX_GLOBALS['custom_colors']) > 1) {
				...
				$response['error'] = sprintf(__('You cannot delete last color scheme!', 'themerex'), $scheme);
		} else
			$response['error'] = sprintf(__('Color Scheme %s not found!', 'themerex'), $scheme);
			...
			foreach ($order as $slug) {
				if (is_array($THEMEREX_GLOBALS['custom_colors'][$slug]) && count($THEMEREX_GLOBALS['custom_colors'][$slug]) > 0) {
					...
					if ($slug == $scheme) {
						...
						$schemes[$new_slug]['title'] .= ' '.__('(Copy)', 'themerex');
					}
				}
			}
			$THEMEREX_GLOBALS['custom_colors'] = $schemes;
...
if ( !function_exists( 'themerex_core_customizer_page' ) ) {
	function themerex_core_customizer_page() {
		...
		if (is_array($THEMEREX_GLOBALS['custom_colors']) && count($THEMEREX_GLOBALS['custom_colors']) > 0) {
			...
			$demo_block = '';
			if (file_exists(themerex_get_file_dir('core/core.customizer/core.customizer.demo.php'))) {
				ob_start();
			...
			$options["partition_schemes"] = array(
				"title" => __('Color schemes', 'themerex'),
				"override" => "customizer",
			...
			foreach ($THEMEREX_GLOBALS['custom_colors'] as $slug=>$scheme) {
				...
				$options["{$slug}-description"] = array(
					"title" => sprintf(__('Color scheme "%s"', 'themerex'), $scheme['title']),
					"desc" => sprintf(__('Specify the color for each element in the schema "%s". After that you will be able to use your color scheme for the entire page, any part thereof and/or for the shortcodes!', 'themerex'), $scheme['title']),
					"override" => "customizer",
					"type" => "info");
				...
				$options["{$slug}-buttons_label"] = array(
					"desc" => __("You can duplicate current color scheme (appear on new tab) or delete it (if not last scheme)", 'themerex'),
					"override" => "customizer",
					"divider" => false,
					"columns" => "4_6 first",
					"type" => "label");
				...
				$options["{$slug}-title_label"] = array(
					"title" => __('Scheme names', 'themerex'),
					"desc" => __('Specify scheme title (to represent this color scheme in the lists) and scheme slug (to use this color scheme in the shortcodes).<br>Attention! If you change scheme title or slug - you must save options (press Save), then reload the page (press F5) after the success saving message appear!', 'themerex'),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				if ($demo_block) {
					$options["{$slug}-demo"] = array(
						"title" => __('Usage demo', 'themerex'),
						"desc" => __('Below you can see the example of decoration of the page with selected colors.', 'themerex') . trim($demo_block),
						"override" => "customizer",
						"type" => "info");
				}
...
if (isset($scheme['accent1'])) {
				$options["{$slug}-accent_info"] = array(
					"title" => __('Accent colors', 'themerex'),
					"desc" => __('Specify colors for theme accented elements (if need). The theme may not use all of the colors.', 'themerex'),
					"override" => "customizer",
					"type" => "info");

				// Accent 1 color
				$options["{$slug}-accent1_label"] = array(
					"title" => __('Accent 1', 'themerex'),
					"desc" => __('Select color for accented elements and their hover state', 'themerex'),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
	...
	if (isset($scheme['accent3'])) {
				...
				$options["{$slug}-text_info"] = array(
					"title" => __('Text and Headers', 'themerex'),
					"desc" => __('Specify colors for the plain text, post info blocks and headers', 'themerex'),
					"override" => "customizer",
					"type" => "info");
				...
				$options["{$slug}-text_label"] = array(
					...
					"desc" => __('Select colors for the text: normal text color, light text (for example - post info) and dark text (headers, bold text, etc.)', 'themerex'),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-inverse_label"] = array(
					"title" => __('Inverse text', 'themerex'),
					"desc" => __('Select colors for inversed text (text on accented background)', 'themerex'),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-inverse_labe2l"] = array(
					"title" => __('Inverse links', 'themerex'),
					"desc" => __('Select colors for inversed links (links on accented background)', 'themerex'),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-block_info"] = array(
					"title" => __('Page/Block decoration', 'themerex'),
					"desc" => __('Specify border and background to decorate whole page (if scheme accepted to the page) or entire block/section.', 'themerex'),
					"override" => "customizer",
					"type" => "info");
				...
				$options["{$slug}-bd_color_label"] = array(
					...
					"desc" => __('Select the border color and it hover state', 'themerex'),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-bg_color_label"] = array(
					...
					"desc" => __('Select the background color and it hover state', 'themerex'),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-bg_image_label"] = array(
					...
					"desc" => __('Select first background image and it display parameters', 'themerex'),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-bg_image2_label"] = array(
					"title" => __('Background image 2', 'themerex'),
					"desc" => __('Select second background image and it display parameters', 'themerex'),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-alter_info"] = array(
					"title" => __('Alternative colors: Highlight areas / Input fields', 'themerex'),
					"desc" => __('Specify colors to decorate inner blocks or input fields in the forms', 'themerex'),
					"override" => "customizer",
					"type" => "info");
				...
				$options["{$slug}-alter_text_label"] = array(
					"title" => __('Text and Headers', 'themerex'),
					"desc" => __('Specify colors for the plain text, post info blocks and headers in the highlight blocks', 'themerex'),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-alter_link_label"] = array(
					...
					"desc" => __('Specify colors for the links in the highlight blocks', 'themerex'),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-alter_bd_color_label"] = array(
					"title" => __('Border color', 'themerex'),
					"desc" => __('Select the border colors for the normal state and for active (focused) field', 'themerex'),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-alter_bg_color_label"] = array(
					"title" => __('Background Color', 'themerex'),
					"desc" => __('Select the background colors for the normal state and for active (focused) field', 'themerex'),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-alter_bg_image_label"] = array(
					"title" => __('Background image', 'themerex'),
					"desc" => __('Select alter background image and it display parameters', 'themerex'),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
		...
		if (is_array($THEMEREX_GLOBALS['custom_fonts']) && count($THEMEREX_GLOBALS['custom_fonts']) > 0) {
			...
			$list_weight = array(
				...
				'100' => __('100 (Light)', 'themerex'), 
				'300' => __('300 (Thin)',  'themerex'),
				'400' => __('400 (Normal)', 'themerex'),
				'500' => __('500 (Semibold)', 'themerex'),
				'600' => __('600 (Semibold)', 'themerex'),
				'700' => __('700 (Bold)', 'themerex')
			);
			...
			foreach ($THEMEREX_GLOBALS['custom_fonts'] as $slug=>$font) {
				...
				if (isset($font['font-size'])) {
					$options["{$slug}-font-size"] = array(
						"title" => $show_titles ? __('Size', 'themerex') : '',
						"desc" => '',
						"divider" => false,
						"columns" => "1_8",
						"std" => "",
				...
				if (isset($font['line-height'])) {
					$options["{$slug}-line-height"] = array(
						"title" => $show_titles ? __('Line height', 'themerex') : '',
						"desc" => '',
						"divider" => false,
						"columns" => "1_8",
						"std" => "",
				...
				if (isset($font['font-weight'])) {
					$options["{$slug}-font-weight"] = array(
						"title" => $show_titles ? __('Weight', 'themerex') : '',
						"desc" => '',
						"divider" => false,
						"columns" => "1_8",
						"std" => "",
				...
				if (isset($font['font-style'])) {
					$options["{$slug}-font-style"] = array(
						"title" => $show_titles ? __('Style', 'themerex') : '',
						"desc" => '',
						"divider" => false,
						"columns" => "1_8",
						"std" => "",
				...
				if (isset($font['margin-top'])) {
					$options["{$slug}-margin-top"] = array(
						"title" => $show_titles ? __('Margin Top', 'themerex') : '',
						"desc" => '',
						"divider" => false,
						"columns" => "1_8",
						"std" => "",
				...
				if (isset($font['margin-bottom'])) {
					$options["{$slug}-margin-bottom"] = array(
						"title" => $show_titles ? __('Margin Bottom', 'themerex') : '',
						"desc" => '',
						"divider" => false,
						"columns" => "1_8",
						"std" => "",
		...
		themerex_options_page_start(array(
			'title' => __('Fonts & Colors', 'themerex'),
			"icon" => "iconadmin-cog",
			"subtitle" => __('Fonts settings & Color schemes', 'themerex'),
			"description" => __('Customize fonts and colors for your site.', 'themerex'),
			'data' => $options,
			'create_form' => true,
...
if (!function_exists('themerex_core_customizer_load_scripts')) {
	function themerex_core_customizer_load_scripts() {
		if (file_exists(themerex_get_file_dir('core/core.customizer/core.customizer.css')))
			themerex_enqueue_style( 'themerex-core-customizer-style',	themerex_get_file_url('core/core.customizer/core.customizer.css'), array(), null);
		if (file_exists(themerex_get_file_dir('core/core.customizer/core.customizer.js')))
			themerex_enqueue_script( 'themerex-core-customizer-script', themerex_get_file_url('core/core.customizer/core.customizer.js'), array(), null );
	}
}
	...
	function themerex_core_customizer_prepare_scripts() {
		...
		<script type="text/javascript">
			jQuery(document).ready(function () {
				...
				THEMEREX_GLOBALS['to_strings'].scheme_delete			= "<?php _e("Delete color scheme", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].scheme_delete_confirm	= "<?php _e("Do you really want to delete this color scheme?", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].scheme_delete_complete	= "<?php _e("Current color scheme is successfully deleted!", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].scheme_delete_failed		= "<?php _e("Error while delete color scheme! Try again later.", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].scheme_copy				= "<?php _e("Copy color scheme", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].scheme_copy_confirm		= "<?php _e("Duplicate this color scheme?", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].scheme_copy_complete		= "<?php _e("Current color scheme is successfully duplicated!", 'themerex'); ?>";
				THEMEREX_GLOBALS['to_strings'].scheme_copy_failed		= "<?php _e("Error while duplicate color scheme! Try again later.", 'themerex'); ?>";
			});
		</script>
		<?php 
	}
...
if (!function_exists('themerex_core_customizer_add_scripts_inline')) {
	//add_action('themerex_action_add_scripts_inline', 'themerex_core_customizer_add_scripts_inline');
	function themerex_core_customizer_add_scripts_inline() {
